    ...
    >
      <div className="pointer-events-none absolute inset-0">
        <div style={{
          ...
          borderRadius: "50%",
          background: `radial-gradient(ellipse, ${hexToRgba(accent, 0.18)} 0%, ${hexToRgba(accent, 0.06)} 45%, transparent 70%)`,
      ...
      >
        <div className="flex flex-col items-center mb-10">
          <div
            ...
            style={{
              boxShadow: `0 0 48px ${hexToRgba(accent, 0.30)}, 0 8px 32px rgba(0,0,0,0.40)`,
            ...
            {step === "pat" && (
              ...
              >
                ...
                <p style={{ fontSize: "0.8125rem", color: "#556080", lineHeight: 1.6, marginBottom: 24 }}>
                  Create a token at <span style={{ color: accent }}>GitHub → Settings → Developer settings → PATs</span> with these scopes:{" "}
                  <code style={{ background: hexToRgba(accent, 0.15), color: accent, padding: "1px 6px", borderRadius: 4, fontSize: 11 }}>repo</code>{" "}
                  <code style={{ background: hexToRgba(accent, 0.15), color: accent, padding: "1px 6px", borderRadius: 4, fontSize: 11 }}>delete_repo</code>{" "}
                  <code style={{ background: hexToRgba(accent, 0.15), color: accent, padding: "1px 6px", borderRadius: 4, fontSize: 11 }}>workflow</code>
                </p>
            ...
            {step === "device-waiting" && deviceFlow && (
              ...
              >
                ...
                <div style={{
                  ...
                  background: `linear-gradient(145deg, ${hexToRgba(accent, 0.14)} 0%, ${hexToRgba(accent, 0.08)} 100%)`,
                  ...
                  <p style={{
                    fontSize: "2.25rem", fontWeight: 800, letterSpacing: "0.22em", color: accent,
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 16, color: "#3A4560", fontSize: "0.75rem" }}>
                  <span style={{
                    ...
                    border: "2px solid #3A4560", borderTopColor: accent,
...
}> = ({ icon, label, sub, onClick, disabled, accent: isAccent }) => {
  ...
  return (
    <button
      ...
      style={{
        ...
        background: hovered
          ? (isAccent ? hexToRgba(storeAccent, 0.16) : "rgba(255,255,255,0.08)")
          : (isAccent ? hexToRgba(storeAccent, 0.09) : "rgba(255,255,255,0.04)"),
        border: isAccent
          ? `1px solid ${hovered ? hexToRgba(storeAccent, 0.40) : hexToRgba(storeAccent, 0.22)}`
          : `1px solid ${hovered ? "rgba(255,255,255,0.14)" : "rgba(255,255,255,0.08)"}`,
        transition: "all 150ms ease",
        boxShadow: hovered && isAccent ? `0 4px 24px ${hexToRgba(storeAccent, 0.18)}` : "none",
      }}
    >
      <span style={{
        width: 48, height: 48, borderRadius: 12, flexShrink: 0,
        display: "flex", alignItems: "center", justifyContent: "center",
        background: isAccent ? hexToRgba(storeAccent, 0.18) : "rgba(255,255,255,0.07)",
        color: isAccent ? storeAccent : "#6B7A9B",
        transition: "all 150ms ease",
      }}>
        {icon}
      </span>
      ...
      <ChevronRight size={16} style={{ color: isAccent ? storeAccent : "#2D3650", flexShrink: 0 }} />
    </button>
  );
};
      ...
      <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
        {icon && (
          <span style={{
            ...
            color: focused ? storeAccent : "#4A5580", transition: "color 160ms",
            display: "flex",
          }}>{icon}</span>
        )}
        <input
          ...
          style={{
            ...
            border: focused ? `1px solid ${hexToRgba(storeAccent, 0.50)}` : "1px solid transparent",
            boxShadow: focused ? `0 0 0 3px ${hexToRgba(storeAccent, 0.10)}` : "none",
...
const PrimaryBtn: React.FC<{
  ...
  <button
    ...
    style={{
      ...
      background: disabled ? hexToRgba(storeAccent, 0.35) : `linear-gradient(135deg, ${storeAccent} 0%, ${storeAccent} 100%)`,
      border: `1px solid ${hexToRgba(storeAccent, 0.35)}`,
      color: "#fff", fontSize: "0.9375rem", fontWeight: 600, cursor: disabled || loading ? "not-allowed" : "pointer",
      display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
      boxShadow: disabled ? "none" : `0 4px 24px ${hexToRgba(storeAccent, 0.30)}, 0 1px 0 rgba(255,255,255,0.12) inset`,
